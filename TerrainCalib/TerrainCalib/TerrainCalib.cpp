...
int main()
{
	Mat src, gaus, hsv, imgthresh, processed;

	src = imread("squares.jpg", CV_LOAD_IMAGE_COLOR);
	imshow("Image", src);
	...
	for (int i = 0; i < data[0].size(); i++) {
		cv::Mat roi = hsv(cv::Range(stoi(data[i][0]) - 20, stoi(data[i][1]) + 20), cv::Range(stoi(data[i][0]) - 20, stoi(data[i][1]) + 20));
		imshow("roi", roi);
		...
		float minHue = mean[0] - stddev[0];
		float maxHue = mean[0] + stddev[0];
