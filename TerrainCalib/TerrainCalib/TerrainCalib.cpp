...
int main()
{
	...
	for (int i = 0; i < 2; i++) {
		...
		if (i == 0) {
			...
			vector<Vec4i> hierarchy;
			findContours(processed, contours1, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE);
			Scalar color(255, 255, 0);  // color of the contour

			write_to_obj_file("temp1.obj", contours1);

			// iterate through each contour.
			for (int i = 0; i < contours1.size(); i++)
			{
				drawContours(src1, contours1, i, color, 5, 8, hierarchy);
			}
			namedWindow("Display window" + to_string(i), CV_WINDOW_AUTOSIZE);
			imshow("Display window" + to_string(i), src1);
		}
